...
import type { UserModule, UserSection } from '@/lib/db-types';
import { moduleLabel, sectionLabel, parseChapterNumber } from '@/lib/db-types';
...
export default function AddLessonPage() {
  ...
  const [existingSections, setExistingSections] = useState<UserSection[]>([]);
  ...
  const [selectedSectionId, setSelectedSectionId] = useState<string>('new');
  ...
  useEffect(() => {
    ...
    db.getUserSections(supabase, userId ?? '', selectedModuleId).then((secs) => {
      setExistingSections(secs);
      setSelectedSectionId(secs.length > 0 ? secs[0].id : 'new');
    });
  }, [selectedModuleId, supabase]);

  // ── Auto-suggest module+section from chapter number ───────────────────────────
  useEffect(() => {
    if (!chapter.trim()) return;
    const { moduleNumber, sectionNumber } = parseChapterNumber(chapter);
    if (moduleNumber != null) {
      const matchingMod = existingModules.find((m) => m.number === moduleNumber);
      if (matchingMod) {
        setSelectedModuleId(matchingMod.id);
        if (sectionNumber) {
          const matchingSec = existingSections.find((s) => s.number === sectionNumber);
          if (matchingSec) setSelectedSectionId(matchingSec.id);
        }
      }
    }
  }, [chapter, existingModules, existingSections]);
  ...
  async function handleGenerate() {
    ...
    if (selectedSectionId === 'new' && (!newSectionNumber.trim() || !newSectionTitle.trim())) return;
    setError(null);
    setStep('generating');
    try {
  ...
  async function handleSave() {
    ...
    try {
      ...
      if (selectedModuleId === 'new') {
        const nextNum = existingModules.length + 1;
        moduleNumber = nextNum;
        const mod = await db.createUserModule(supabase, userId, {
          number: nextNum,
          title: newModuleTitle.trim(),
          position: existingModules.length,
        });
        if (!mod) throw new Error('Failed to create module');
        moduleId = mod.id;
      } else {
        moduleId = selectedModuleId;
        moduleNumber = existingModules.find((m) => m.id === moduleId)?.number ?? undefined;
      }

      // 2. Resolve section
      let sectionId: string;
      if (selectedSectionId === 'new') {
        const secCount = existingSections.length;
        const sec = await db.createUserSection(supabase, {
          user_id:  userId,
          module_id: moduleId,
          number:   newSectionNumber.trim(),
      ...
      await db.createUserFlashcards(supabase, keptCards.map((c) => ({
        user_id:   userId,
        module_id: moduleId,
        lesson_id: lesson.id,
        question:  c.question,
      ...
      await db.createUserQuizQuestions(supabase, keptQuestions.map((q) => ({
        user_id:       userId,
        module_id:     moduleId,
        lesson_id:     lesson.id,
        prompt:        q.prompt,
  ...
  const canGenerate =
    ...
    (selectedSectionId !== 'new' || (newSectionNumber.trim().length > 0 && newSectionTitle.trim().length > 0));

  const selectedModule  = existingModules.find((m) => m.id === selectedModuleId);
      ...
      <div className="max-w-lg mx-auto px-5 py-6 space-y-6">
          ...
          <div className="space-y-5">
            ...
            <div className="space-y-1.5">
              ...
              <select
                ...
                disabled={selectedModuleId === 'new'}
                className="w-full px-3 py-2.5 rounded-md border border-rule bg-card font-sans text-sm text-ink focus:outline-none focus:border-accent transition-colors disabled:opacity-50"
              >
              ...
              {selectedSectionId === 'new' && (
                <div className="grid grid-cols-5 gap-2 mt-2">
                  <input
                    ...
                    className="col-span-2 px-3 py-2.5 rounded-md border border-rule bg-card font-sans text-sm text-ink placeholder:text-ink-2/50 focus:outline-none focus:border-accent transition-colors"
                  />
                  <input
                    type="text"
                    ...
                    className="col-span-3 px-3 py-2.5 rounded-md border border-rule bg-card font-sans text-sm text-ink placeholder:text-ink-2/50 focus:outline-none focus:border-accent transition-colors"
                  />
                </div>
              )}
            </div>
            ...
            <div className="grid grid-cols-5 gap-3">
              ...
              <div className="space-y-1.5 col-span-3">
                ...
                <input
                  type="text"
                  placeholder="The Start of the Mortgage Industry"
                  value={lessonTitle}
          ...
          <div className="space-y-6">
            ...
            <div className="border border-rule rounded-md bg-card px-4 py-3 space-y-1">
              <p className="font-sans text-xs text-ink-2">
                ...
                {selectedModuleId === 'new' ? newModuleTitle : (selectedModule ? moduleLabel(selectedModule) : '')}
              </p>
              <p className="font-sans text-xs text-ink-2">
                <span className="font-medium text-ink">Section:</span>{' '}
                {selectedSectionId === 'new' ? `${newSectionNumber} ${newSectionTitle}` : (selectedSection ? sectionLabel(selectedSection) : '')}
              </p>
              <p className="font-sans text-xs text-ink-2">
                ...
                {flashcards.map((card, i) => (
                  ...
                  >
                    <div className="flex items-start justify-between gap-3">
                      <span className="font-sans text-[10px] font-semibold uppercase tracking-wider text-ink-2">Card {i + 1}</span>
                      <button
                    ...
                    <div className="space-y-2">
                      <textarea rows={2} value={card.question} onChange={(e) => updateCard(i, 'question', e.target.value)} placeholder="Question" className="w-full px-3 py-2 rounded border border-rule bg-paper font-sans text-sm text-ink placeholder:text-ink-2/50 focus:outline-none focus:border-accent transition-colors resize-none" />
                      <textarea rows={2} value={card.answer} onChange={(e) => updateCard(i, 'answer', e.target.value)} placeholder="Answer" className="w-full px-3 py-2 rounded border border-rule bg-paper font-sans text-sm text-ink placeholder:text-ink-2/50 focus:outline-none focus:border-accent transition-colors resize-none" />
                    </div>
                  </div>
                ))}
              </div>
            ...
            <section className="space-y-3">
              ...
              <div className="space-y-4">
                {quizQuestions.map((q, qi) => (
                  ...
                  >
                    <div className="flex items-start justify-between gap-3">
                      <span className="font-sans text-[10px] font-semibold uppercase tracking-wider text-ink-2">Q{qi + 1}</span>
                      <button onClick={() => toggleQuestion(qi)} className={`font-sans text-[10px] font-medium transition-colors ${q.keep ? 'text-accent hover:text-ink' : 'text-ink-2 hover:text-accent'}`}>
                        {q.keep ? 'Remove' : 'Keep'}
                      </button>
                    </div>
                    <textarea rows={2} value={q.prompt} onChange={(e) => updateQuestion(qi, 'prompt', e.target.value)} placeholder="Question prompt" className="w-full px-3 py-2 rounded border border-rule bg-paper font-sans text-sm text-ink placeholder:text-ink-2/50 focus:outline-none focus:border-accent transition-colors resize-none" />
                    <div className="space-y-1.5">
                      <p className="font-sans text-[10px] text-ink-2 font-medium">Choices — select the correct answer</p>
                      {q.choices.map((choice, ci) => (
                        <div key={ci} className="flex items-center gap-2">
                          <input type="radio" name={`correct-${qi}`} checked={q.correctIndex === ci} onChange={() => updateCorrectIndex(qi, ci)} className="shrink-0 accent-sage" />
                          <span className="font-sans text-xs text-ink-2 tabular-nums w-4 shrink-0">{String.fromCharCode(65 + ci)}.</span>
                          <input type="text" value={choice} onChange={(e) => updateChoice(qi, ci, e.target.value)} className="flex-1 px-2 py-1.5 rounded border border-rule bg-paper font-sans text-sm text-ink focus:outline-none focus:border-accent transition-colors" />
                          {q.correctIndex === ci && <span className="font-sans text-[10px] text-sage font-medium shrink-0">✓</span>}
                        </div>
                      ))}
                    </div>
                    <textarea rows={2} value={q.explanation} onChange={(e) => updateQuestion(qi, 'explanation', e.target.value)} placeholder="Explanation" className="w-full px-3 py-2 rounded border border-rule bg-paper font-sans text-sm text-ink placeholder:text-ink-2/50 focus:outline-none focus:border-accent transition-colors resize-none" />
                  </div>
                ))}
              </div>
            </section>
        ...
        {step === 'done' && savedLessonId && savedModuleId && (
          <div className="border border-rule rounded-md bg-card p-6 space-y-5">
            ...
            <div className="space-y-2">
              <a href={`/learn/${savedModuleId}/${savedLessonId}`} className="block w-full py-2.5 rounded-md bg-accent text-card font-sans text-sm font-medium text-center hover:opacity-90 transition-opacity">
                Read Lesson →
              </a>
              <a href={`/flashcards?lesson=${savedLessonId}`} className="block w-full py-2.5 rounded-md border border-rule font-sans text-sm font-medium text-ink-2 text-center hover:border-ink-2 hover:text-ink transition-colors">
                Study Flashcards
              </a>
              <button
                onClick={() => {
                  setStep('form'); setRawContent(''); setLessonTitle('');
                  setChapter(''); setWhyItMatters(''); setError(null);
                  setSavedLessonId(null); setSavedModuleId(null);
                  setSelectedSectionId(existingSections.length > 0 ? existingSections[0].id : 'new');
                }}
